node {
    ...
    stage('Deploy') {
        if (env.BRANCH_NAME == "main") {
            ...
            sh "docker run --restart=always -e API_KEY=${API_KEY} -e CLIENT_ID=${CLIENT_ID} -e CLIENT_SECRET=${CLIENT_SECRET} -d -p 45002:8888 --name destiny2-bounties-helper -v /volume1/docker/config_files/certs:/app/files slhad/destiny2_bounties_helper"
            }
        }
    }
}